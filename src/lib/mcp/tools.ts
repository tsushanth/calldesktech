...
// Args are typed `any` throughout — each tool's real shape is already described by its zod
// inputSchema below, not by a static type here; threading real per-tool types through would need
// pulling in the MCP SDK's registerTool signature, a bigger change than this file's actual risk
// warrants. (Previously blanket `@ts-nocheck`, which also silently disabled real type-checking
// for the rest of the file — replaced with explicit `any` at each real dynamic boundary instead,
...
const fail = (err: any) => ({ isError: true, content: [{ type: 'text', text: err instanceof Error ? err.message : String(err) }] });
const run = (fn: any) => async (args: any) => { try { return ok(await fn(args)); } catch (e: any) { return fail(e); } };
...
server.registerTool('create_agent', { description: 'Create an agent (no flow yet — publish a version next).', annotations: WRITE, inputSchema: { name: z.string().min(1), mode: z.enum(['simple', 'advanced']).optional() } }, run(async (a: any) => api('POST', `/tenants/${await tenant()}/agents`, a)));
server.registerTool('get_agent', { description: 'Get one agent.', annotations: READ, inputSchema: { agentId: z.string() } }, run((a: any) => api('GET', `/agents/${a.agentId}`)));
server.registerTool('rename_agent', { description: 'Rename an agent.', annotations: WRITE, inputSchema: { agentId: z.string(), name: z.string().min(1) } }, run((a: any) => api('PATCH', `/agents/${a.agentId}`, { name: a.name })));
...
server.registerTool('list_agent_environments', { description: 'List an agent’s staging/production environments and which version each currently points to (null if nothing promoted yet).', annotations: READ, inputSchema: { agentId: z.string() } }, run((a: any) => api('GET', `/agents/${a.agentId}/environments`)));
server.registerTool('promote_agent_environment', { description: 'Promote a version into staging or production. Every number/batch call routed to that environment picks up the new version immediately — no re-routing needed. Rolling back is promoting an older version again.', annotations: WRITE, inputSchema: { agentId: z.string(), name: z.enum(['staging', 'production']), versionId: z.string() } }, run((a: any) => api('POST', `/agents/${a.agentId}/environments/${a.name}/promote`, { versionId: a.versionId })));
server.registerTool('place_call', { description: 'Place a REAL outbound phone call (costs money) from one of your numbers using its outbound agent.', annotations: COSTS, inputSchema: { phoneNumberId: z.string(), toNumber: z.string().describe('E.164, e.g. +14155550123') } }, run((a: any) => api('POST', `/phone-numbers/${a.phoneNumberId}/call`, { toNumber: a.toNumber })));
server.registerTool('list_calls', { description: 'List recent calls.', annotations: READ, inputSchema: { limit: z.number().int().min(1).max(200).optional() } }, run(async (a: any) => api('GET', `/tenants/${await tenant()}/calls?limit=${a.limit ?? 25}`)));
...
server.registerTool('create_webhook', { description: 'Register a webhook. Events: call.started, call.completed, call.analyzed, call.transferred. Returns the signing secret.', annotations: WRITE, inputSchema: { url: z.string().url(), events: z.array(z.enum(['call.started', 'call.completed', 'call.analyzed', 'call.transferred'])).optional() } }, run(async (a: any) => api('POST', `/tenants/${await tenant()}/webhooks`, a)));
server.registerTool('delete_webhook', { description: 'Delete a webhook.', annotations: DESTROY, inputSchema: { webhookId: z.string() } }, run(async (a: any) => api('DELETE', `/tenants/${await tenant()}/webhooks/${a.webhookId}`)));

// ---- crm
server.registerTool('lookup_hubspot_contact', { description: 'CRM→us lookup: find a caller\'s HubSpot contact by phone (if the tenant has connected HubSpot in Integrations) and return fields usable as call-time dynamic variables, e.g. crm_company_name.', annotations: READ, inputSchema: { phone: z.string().describe('E.164, e.g. +14155550123') } }, run(async (a: any) => api('GET', `/tenants/${await tenant()}/crm/hubspot/lookup?phone=${encodeURIComponent(a.phone)}`)));

// ---- analytics
server.registerTool('get_analytics', { description: 'Call analytics by day.', annotations: READ, inputSchema: { days: z.enum(['7', '30', '90']).default('30') } }, run(async (a: any) => api('GET', `/tenants/${await tenant()}/analytics?days=${a.days}`)));
server.registerTool('get_qa_overview', { description: 'QA scores, resolution rate and transfer metrics.', annotations: READ, inputSchema: { days: z.enum(['7', '30', '90']).default('30') } }, run(async (a: any) => api('GET', `/tenants/${await tenant()}/qa/overview?days=${a.days}`)));
